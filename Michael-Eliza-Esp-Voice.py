#Eliza in Spanish, Updated by Michael Mason-D'Croz
# with added voice chat
...
def playAudio(text, language='es'):
    voz = gTTS(text, lang=language)
    voz.save('voz1.mp3')
    playsound('voz1.mp3')
    os.remove('voz1.mp3')
...
def main():
    i = 0
    saludo = "Hola ¿Cómo te sientes hoy?¿Con resaca?"
    playAudio(saludo)
    while True:
        ...
        with sr.Microphone() as source:
            ...
            try:
                ...
                oops = "No te entiendo. Reempezarlo de nuevo..."
                print(oops)
                playAudio(oops)
                engine.runAndWait()
        if incoming:
            statement = incoming.lower()
            response = analyze(statement)
            print(response)
            playAudio(response)
            engine.runAndWait()
        elif incoming == "quit":
            playAudio('Hasta luego')
            break
